...
| Coverage surface (F14-F19) | structural fail-closed | `build_denial` raises `PhysicianSignoffRequired` without a physician token — no demo path supplies one and no override exists; `determine()` approves or escalates, never denies; quote-anchored grounding; hashed, versioned criteria packs (DRAFT status surfaced verbatim). 26 dedicated tests. |
